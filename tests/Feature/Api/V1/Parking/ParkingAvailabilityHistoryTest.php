...
it('paginates availability history', function () {
    ...
    $this
        ->getJson(
            "/api/v1/parking/facility:{$facility->id}/availability/history?per_page=2&page=1",
        )
        ->assertOk()
...
it('returns the requested availability history page', function () {
    ...
    $this
        ->getJson(
            "/api/v1/parking/facility:{$facility->id}/availability/history?per_page=2&page=2",
        )
        ->assertOk()
        ->assertJsonCount(2, 'data')
...
it('returns 404 when the parking entity does not exist', function () {
    $this
        ->getJson(
            '/api/v1/parking/facility:999999/availability/history',
        )
        ->assertNotFound();

    $this
        ->getJson(
            '/api/v1/parking/street:999999/availability/history',
        )
        ->assertNotFound();
});
...
it('validates pagination parameters', function () {
    ...
    $this
        ->getJson(
            "/api/v1/parking/facility:{$facility->id}/availability/history?per_page=0",
        )
        ->assertUnprocessable();

    $this
        ->getJson(
            "/api/v1/parking/facility:{$facility->id}/availability/history?page=0",
        )
        ->assertUnprocessable();

    $this
        ->getJson(
            "/api/v1/parking/facility:{$facility->id}/availability/history?per_page=101",
        )
        ->assertUnprocessable();
});
